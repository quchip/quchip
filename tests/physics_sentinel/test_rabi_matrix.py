...
def _run_rabi(*, frame: str, approximation) -> tuple[np.ndarray, np.ndarray]:
    q = DuffingTransmon(freq=5.0, anharmonicity=-0.25, levels=3, label="q")
    drive = ChargeDrive(target=q)
    chip = Chip([q], frame=frame, approximation=approximation)
...
def test_rabi_matrix_hits_pi_and_two_pi_points(frame: str, approximation) -> None:
    """Rabi drive reaches near-full inversion at the pi time and returns near zero at the 2pi time."""
    tlist, p1 = _run_rabi(frame=frame, approximation=approximation)
...
def test_rabi_matrix_configs_agree() -> None:
    ...
    reference_time, reference = _run_rabi(frame="rotating", approximation=RWA())

    for approximation in (RWA(), Exact()):
        for frame in ("lab", "rotating"):
            tlist, p1 = _run_rabi(frame=frame, approximation=approximation)
            npt.assert_allclose(tlist, reference_time)
            # Measured: max deviation from the rotating/approximation=RWA() reference is ~3.4e-4 (lab
            # frame, approximation=Exact()), dominated by the Bloch-Siegert-scale counter-rotating shift;
            # atol=5e-3 keeps a ~15x margin over that.
            npt.assert_allclose(
                p1,
                reference,
                atol=5e-3,
                err_msg=f"Rabi population drifted for frame={frame}, {type(approximation).__name__}",
            )
